var myStepDefinitionsWrapper = function () {
    ...
    this.When(/^User enter the Title$/, function (callback) {
        merchandiseHierarchy.getTitleTextBox().sendKeys(titleText);
        callback();
    });

    this.When(/^User clicks on  culture button$/, function (callback) {
        merchandiseHierarchy.getTitleCultureIcon().click().then(function () {
            callback();
        });
    });

    this.Then(/^A pop up will open with culture and value	field$/, function (callback) {
    ...
    this.Then(/^default culture and value will get displayed in the existing table below culture drop down$/, function (callback) {
    ...
    this.When(/^User leave the Title field as blank$/, function (callback) {
        merchandiseHierarchy.getTitleTextBox().sendKeys(" ").then(function () {
            callback();
        });
    });

    this.Then(/^A pop up will open with culture and value$/, function (callback) {
        dynamicAttr.getCultureDropdown().isDisplayed().then(function (result) {
            assert.equal(result, true);
        });
        dynamicAttr.getCultureValueTextBox().isDisplayed().then(function (result) {
    ...
    this.Then(/^The system’s default culture will get displayed$/, function (callback) {
        dynamicAttr.getCultureDropdown().getAttribute(settings.constants.VALUE).then(function (result) {
            assert.isNotNull(result);
            callback();
        });
    ...
    this.Given(/^culture pop up is opened for Title field from Create Merchandise Hierarchy screen$/, function (callback) {
        commonlib.open();
        this.mode = 'create';
        merchandiseHierarchy.openViewScreen().then(function () {
            actionButtons.getAddButton().click().then(function () {
    ...
    this.When(/^click on the drop down in the pop up$/, function (callback) {
    ...
    this.Then(/^drop down will get displayed with different cultures$/, function (callback) {
    ...
    this.Then(/^Each value will display with relevant flag and country value in the parenthesis$/, function (callback) {
    ...
    this.When(/^User select the language from the culture drop down$/, function (callback) {
        dynamicAttr.getCultureDropdown().clear().then(function () {
            dynamicAttr.getCultureDropdown().click().then(function (result) {
    ...
    this.Then(/^Country will appear with the corresponding flag in the Parenthesis$/, function (callback) {
        dynamicAttr.getCultureDropdown().getAttribute(settings.constants.VALUE).then(function (result) {
            assert.include(result, settings.constants.LEFTPARENTHESIS);
            assert.include(result, settings.constants.RIGHTPARENTHESIS);
            callback();
    ...
    this.Then(/^User will be able to select the language from culture drop down$/, function (callback) {
        dynamicAttr.getCultureDropdown().clear().then(function () {
            dynamicAttr.getCultureDropdown().click().then(function (result) {
                dynamicAttr.getCultureDropDownElements().first().click().then(function () {
                    callback();
    ...
    this.Then(/^User will be able to define the value for the particular language$/, function (callback) {
        dynamicAttr.getCultureValueTextBox().sendKeys(settings.constants.DYNATTRNAMESAMPLETEXT).then(function (result) {
            callback();
        });
    });

    this.When(/^User is adding the translation values$/, function (callback) {
        dynamicAttr.getCultureDropdown().clear().then(function () {
            dynamicAttr.getCultureDropdown().click().then(function (result) {
                dynamicAttr.getCultureDropDownElements().first().click().then(function () {
    ...
    this.When(/^Values gets added in the existing values$/, function (callback) {
    ...
    this.Then(/^All the translation values will be in editable mode$/, function (callback) {
    ...
    this.Then(/^User will be able to edit existing value$/, function (callback) {
        dynamicAttr.getCultureTableFirstTextArea().isEnabled().then(function (result) {
    ...
    this.When(/^User define value for that language$/, function (callback) {
        dynamicAttr.getCultureValueTextBox().sendKeys(settings.constants.DYNATTRNAMESAMPLETEXT).then(function () {
            callback();
        });
    });
    ...
    this.Then(/^language and value will get added in bottom of the existing value$/, function (callback) {
        dynamicAttr.getCultureTableInAddEditMode().count().then(function (length) {
            if (length > 0) {
    ...
    this.Then(/^User will notice that the add was performed$/, function (callback) {
        dynamicAttr.getCultureTableInAddEditMode().count().then(function (length) {
            if (length > 0) {
                dynamicAttr.getCultureTableInAddEditMode().last().getInnerHtml().then(function (result) {
                    assert.include(result, expectedResultDictionary.cultureDropdownValue);
    ...
    this.Given(/^Culture pop-up is opened for Title field from Merchandise Hierarchy screen$/, function (callback) {
        commonlib.open();
        this.mode = 'create';
        merchandiseHierarchy.openViewScreen().then(function () {
            actionButtons.getAddButton().click().then(function () {
                commonlib.waitForElement(merchandiseHierarchy.getTitleCultureIcon()).then(function () {
                    merchandiseHierarchy.getCodeTextBox().clear().sendKeys('DummyCode').then(function() {
                        merchandiseHierarchy.getTitleCultureIcon().click().then(function () {
                            callback();
                        });
                    });
    ...
    this.Given(/^Culture pop-up is opened for Title field from Merchandise Hierarchy screen in Edit Mode$/, function (callback) {
        commonlib.open();
        this.mode = 'Edit';
        merchandiseHierarchy.openViewScreen().then(function () {
    ...
    this.Then(/^culture will be displayed in the drop list as disabled\.$/, function (callback) {
        dynamicAttr.getCultureDropdown().clear().then(function () {
            dynamicAttr.getCultureDropdown().click().then(function (result) {
    ...
    this.When(/^User define value for that language to the max (\d+) characters$/, function (arg1, callback) {
        callback.pending();
    });
        ...
        dynamicAttr.getCultureIconInAddEditMode().click().then(function () {
            dynamicAttr.getCultureTableInAddEditMode().count().then(function (length) {
                if (length > 0) {
                    dynamicAttr.getCultureTableInAddEditMode().count().then(function (result) {
                        assert.isNotNull(result);
                        commonlib.waitForAllElements(dynamicAttr.getCultureCancelButton()).then(function () {
                            dynamicAttr.getCultureCancelButton().click().then(function () {
    ...
    this.When(/^user click on the delete button of any existing translation value$/, function (callback) {
        merchandiseHierarchy.getCultureDropdown().clear().then(function () {
            merchandiseHierarchy.getCultureDropdown().click().then(function (result) {
                merchandiseHierarchy.getCultureDropDownElements().first().click().then(function () {
                    merchandiseHierarchy.getCultureValueTextBox().sendKeys(settings.constants.DYNATTRNAMESAMPLETEXT).then(function () {
                        merchandiseHierarchy.getCultureAddButton().click().then(function () {
                            merchandiseHierarchy.getCultureTableInAddEditMode().count().then(function(length) {
                                assert.isAbove(length, 0, 'Merchandise Hierarchy Title Translation value is not added');
                                var firstRow = merchandiseHierarchy.getCultureTableInAddEditMode().first();
                                firstRow.click().then(function () {
                                    merchandiseHierarchy.getCultureTableDeleteIcon(firstRow).click().then(function (result) {
                                        callback();
                                    });
                                });
                            });
    ...
    this.Then(/^System will display a Warning Message when trying to delete a Hierarchy Title for an new Merchandise Hierarchy group$/, function (callback) {
        commonlib.waitForElement(commonlib.getNEPAlertYes()).then(function(){
            commonlib.getNEPAlertYes().click().then(function(){
                merchandiseHierarchy.getCultureCancelButton().click().then(function () {
                    merchandiseHierarchy.onCancel().click().then(function () {
                        commonlib.waitForElement(commonlib.getNEPAlertYes()).then(function(){
                            commonlib.getNEPAlertYes().click().then(function(){
                                callback();
    ...
    this.When(/^User clicks on  culture button for Title field$/, function (callback) {
        merchandiseHierarchy.getTitleCultureIcon().click().then(function () {
            callback();
        });
    });
    ...
    this.Then(/^User should be able to view all the existing transalation values for the Title field$/, function (callback) {
